...
class SessionData(BaseModel):
    ...
    Contains the essential fields for analytics visualization.
...
async def get_chat_sessions(limit: int = 50, db: Session = Depends(get_db)):
    """
    Retrieve chat sessions for analytics dashboard.
    
    Takes an optional limit query parameter (default 50), queries the last N rows 
    from chat_logs ordered by created_at DESC, and returns JSON array reversed 
    so it's oldest→newest with fields { ts, tokens, sentiment }.
    ...
    Returns:
        list[SessionData]: List of session data points ordered oldest→newest
        
    Author: Brian Letort
    """
    try:
        # Query last N rows from chat_logs ordered by created_at DESC
        chat_logs = db.query(ChatLog).order_by(ChatLog.created_at.desc()).limit(limit).all()
        
        # Format response with required fields { ts, tokens, sentiment }
        sessions = [
            SessionData(
        ...
        # Reverse the array so it's oldest→newest
        sessions.reverse()
